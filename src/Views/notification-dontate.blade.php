<!-- notification-dontate.blade.php -->
<div id="notif29" class="notifBox">
    <div class="notifX"><a href="/map?notif=29&notifOnly=1" target="hidFrame" onClick="return hideNotif(29);"
        ><img src="/images/spacer.gif" border=1 height=15 width=15 ></a></div> 
    <div class="notifDesc condensed"><table border=0 cellpadding=0 cellspacing=0 ><tr>
        <td rowspan=2 ><img src="/images/notif-heart.jpg" border=0 ></td>
        <td class="notifNote" rowspan=2 style="padding-right: 30px;">
            <div class="f18" style="letter-spacing: -0.01em; margin-top: -9px; height: 35px;"><nobr>
                Please consider donating a couple bucks</nobr></div>
            <div class="f10" style="font-weight: normal; font-family: Helvetica, Arial, sans-serif;"><nobr>
                After 8 years of BurnerMap, we've just gone 
                <a href="https://github.com/wikiworldorder/burnermap" target="_blank" style="font-size: 10pt;"
                    >open source</a>!</nobr></div>
        </td>
        <td style="margin: 0px; text-align: left; position: relative;">
            <div class="absDiv" style="top: -4px; left: -4px;">
                <a href="https://www.patreon.com/morganlesko" target="_blank" 
                    ><div style="background: #fff9eb; margin: 0px; -moz-border-radius: 10px; border-radius: 10px;
                    padding: 0px;"><img src="/images/patreon_navigation_logo_mini_orange.png" border=0 height=36 
                        style="margin: 4px 5px -2px 7px;"></div></a>
            </div>
            <div class="absDiv" style="top: -4px; left: 72px;">
                <form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
                <input type="hidden" name="cmd" value="_s-xclick">
                <input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----
                ">
                <input type="image" src="https://www.paypalobjects.com/en_US/i/btn/btn_donate_LG.gif" border="0" name="submit" 
                    alt="PayPal - The safer, easier way to pay online!">
                <img alt="" border="0" src="https://www.paypalobjects.com/en_US/i/scr/pixel.gif" width="1" height="1">
                </form>
            </div>
            <div class="absDiv" style="top: -3px; left: 204px;">
                <a href="/bitcoin-addy?print=1" rel="shadowbox"
                    ><img src="/images/bitcoin.png" height=26 border=0 ></a>
            </div>
            <div class="absDiv" style="top: 27px; left: 69px; font-size: 8pt;">1BLcNNL4bv8fQ8d4XN9KXt7T2LU2pjLMN5</div>
        </td></tr>
    </table></div>
</div>